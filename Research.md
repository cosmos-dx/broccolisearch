...
- **H1:** A per-query optimizer strictly dominates the best fixed strategy on **latency-at-fixed-recall** when the workload contains a mix of query types (keyword-heavy, semantic, filtered).
  ...
  > **Status: supported, once "recall" was defined correctly — and only on mixed workloads.** As first measured, H1 was falsified on BEIR: the optimizer was quality-dominated by `hybrid_rrf` (−0.046 and −0.027 nDCG@10) and never selected it. The defect was in the hypothesis's own terms — "fixed recall" had been operationalised as **operator fidelity**, which an exact vector scan satisfies at 1.0, making fusion unreachable. With relevance coverage measured per index (§7.3) the optimizer now matches the best fixed strategy's nDCG on both datasets at the default target, and `recall` traces a real quality/cost curve.
  >
  > The **cost** half of H1 does not reproduce on BEIR, and the reason is a precondition the hypothesis already states: a per-query routing win requires a workload of *mixed query shapes*. SciFact and NFCorpus are homogeneous, so there is nothing to route and the optimizer correctly lands on one plan for the whole workload.
  >
  > **The cost half therefore remains untested on real judged data.** It holds on the synthetic mixed workload (`examples/demo.py`), where the learned policy routes per query instead of hedging into fusion, but a synthetic workload is exactly the place this claim is easiest to accidentally construct in its own favour. Settling it needs a judged corpus whose query *shapes* genuinely differ — short identifier lookups, paraphrases, and natural-language descriptions of the same intent — which neither BEIR dataset provides.
...
- **H4:** A learned policy improves over rules **primarily on tail/ambiguous query classes**, not on clear-cut ones (RQ4).
  ...
  > **Status: supported in the cost dimension, unsupported in the quality dimension.** `LearnedPolicy` is built and measured (README). Trained on half of SciFact/NFCorpus and scored on the held-out half, it finds a materially better cost/quality point than any fixed strategy — on SciFact it *dominates* the vector baseline (0.675 vs 0.673 nDCG at 842 vs 850 work) and reaches 97.7% of fusion's quality for 5.5x less work. It does not exceed fusion's quality, so the "improves over rules" claim holds for **cost at near-equal quality**, not for quality itself.
  >
  > Three estimators failed first, and the reasons are the transferable result:
  > 1. Learning **recall@k** degenerates to "always fuse" — fusion retrieves strictly more, so a recall-maximiser can never prefer the cheap plan.
  > 2. Learning nDCG over **absolute-df buckets** lost to the rules outright. Absolute document counts are not comparable across corpus sizes, so on a large corpus every query fell into one bucket and the policy had nothing to condition on. Bucketing by corpus *fraction* fixed it.
  > 3. Learning **unpaired** means could not resolve 0.02–0.08 nDCG differences at ~150 training queries; the paired per-query difference cancels query difficulty, which is the dominant variance component, and makes those gaps measurable.
  >
  > The tail/ambiguous-class part of H4 remains **untested at scale**: the discriminating experiment is MS MARCO, where judged queries are ~1000x more plentiful and the buckets would not be data-starved.
...
3. **Cost differences below the model's own error must not decide plans.** A workload of short identifier-style queries exposed this: two plans priced at 850 and 851 work units, and the planner took the 0.1% saving while giving up almost all of the quality. With ~17% median cost-model error, any argmin over latency is partly an argmin over noise. `COST_TIE_BAND` now breaks near-ties toward the plan using more evidence, which is a floor rather than a solution: the principled version prices the *marginal* value of each retrieval source instead of banding the total.

4. **Fusion recall composition.** Estimating the recall of an RRF/weighted fusion of two imperfect candidate sets, cheaply, before running it.

   > **Resolved for the judgment-free case; the residual is understood.** Measured on BEIR (see H1), fusion was the best strategy on every real dataset and the optimizer never picked it. The union model `1 - Π(1 - rᵢ)` was not wrong arithmetically — it was being fed the wrong `rᵢ`. Each index reported how faithfully it computed *its own* similarity function, which says nothing about whether that function matches the corpus's judgments.
   >
   > The fix does not need labels. Fusion's gain comes from the indexes **disagreeing**, and disagreement is directly observable: sample documents as queries, run the fusion, and measure what fraction of the fused top-k each index would have returned alone (`Index._measure_index_agreement`). That fraction is the missing `rᵢ` multiplier.
   >
   > One subtlety cost an iteration and is worth recording. Scoring raw **Jaccard disagreement** between the two indexes seems equivalent and is not: it cannot distinguish "these indexes are complementary" from "these indexes disagree because one is wrong". On a corpus where the vector index alone is already perfect, a weak lexical index dragged the estimate down and the planner began fusing everything. Comparing against the **fused result** is immune, because an index that contributes nothing useful also contributes nothing to the fusion.
   >
   > The residual: no judgment-free proxy can tell that two *different* returned sets are equally relevant when documents are interchangeable. Where relevant sets are large and substitutable, this measure is conservative and over-prices fusion. Judgments settle it, which is what `LearnedPolicy` is for — and on the synthetic demo, that is exactly the gap between the two adaptive rows (9607 work hedging vs 8200 routing).
...
7. **Reproducibility of calibration.** Constants fitted from wall-clock timings are themselves random variables. With least-squares fitting, recalibrating an unchanged corpus moved constants by up to four orders of magnitude, which silently moved plan choice. A robust (Theil–Sen) fit contains this, but *how much calibration variance is tolerable before plan choice becomes unstable* is unquantified — and it bounds RQ1/H3.
   ...
   > **Partly a property of the language, not the method.** Porting the lexical scan to Rust narrowed the spread of per-calibration medians from 6.9%–40.3% to 9.8%–29.7% over nine independent builds each, with nothing about the fitting changed. Some of what looked like estimator variance was the interpreter's own variance being fitted faithfully. This does not remove the open problem — the remaining spread is still wide enough to matter — but it says part of the answer is "make the operator predictable", not only "make the estimator robust".

8. **How much of a cost model's error is the measurement instrument?** The model was consistently worst on the *cheapest* operators (49% on sub-0.05ms keyword scans against 12% on vector plans), which admits two readings: the model is wrong at small scales, or timing an interpreter at that resolution is. Replacing the Python scan with a native one and changing nothing else cut keyword error from **43.5% to 8.9%**, with the untouched vector path flat as a control — so it was mostly the instrument.

   > The generalisable claim is uncomfortable: **a cost model's reported accuracy is partly a statement about the runtime it is measuring**, and cross-system comparisons of "cost model error" are not obviously meaningful. The residual also names the next question. Filtered keyword queries went the *other* way (24.7% → 33.0%), because the FFI boundary introduced real work — marshalling the filter domain — that the model has no term for. Faster operators do not automatically mean better estimates; they mean the estimate must describe a different machine.
