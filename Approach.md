...
- **An equivalence test must be shown to be capable of failing.** The Rust/Python agreement test passed on its first run for the wrong reason: it queried `"gardening"` against an index that had stemmed the term to `"garden"`, so both backends returned nothing and agreed perfectly about nothing. Two habits catch this — assert the probe actually retrieves something before comparing, and *mutate the implementation on purpose* (perturbing a BM25 constant by 1e-10) to confirm the test goes red. A green equivalence test is worthless until you have seen it fail.
- **Keep a control in the experiment.** When the native scan improved keyword error 43.5% → 8.9%, the claim was only credible because the vector path — which was not touched — stayed flat at 17.7% vs 17.5%. Without an untouched row in the same table, "everything improved" and "the measurement drifted" look identical.
- **A speedup can move error in both directions; report the row that got worse.** The same port that fixed keyword estimates made `filtered_kw` worse (24.7% → 33.0%), because it introduced FFI marshalling the cost model has no term for. Averaging the two into one improved headline would have hidden a new defect behind a real win.
